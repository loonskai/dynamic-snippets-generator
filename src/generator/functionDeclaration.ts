...
const _functionDeclaration = (rawCodeStr: string): string => {
  ...
  traverse(ast, {
    FunctionDeclaration(path) {
      ...
      const newId = generateNode.identifier(getNamedPlaceholder(counter.value, initialName));
      ...
      const newFunctionDeclaration: any = t.functionDeclaration(newId, newParams, body);
