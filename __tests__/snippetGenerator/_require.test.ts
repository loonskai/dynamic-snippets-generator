...
import abbreviationIDs from '../../src/constants/abbreviationIDs';

describe('require statement', () => {
  it('default require', () => {
    expect(
      generate(abbreviationIDs.RQR, "const package = require('package');"),
    ).toEqual("const ${1:package} = require('${2:package}');");
  });

  it('require with custom name', () => {
    expect(
      generate(abbreviationIDs.RQR, "const customName = require('package');"),
    ).toEqual("const ${1:customName} = require('${2:package}');");
  });

  it('require with no object properties', () => {
    expect(
      generate(abbreviationIDs.RQR, "const {} = require('package');"),
    ).toEqual("const { $1 } = require('${2:package}');");
  });

  it('require with 1 object properties', () => {
    expect(
      generate(abbreviationIDs.RQR, "const { a } = require('package');"),
    ).toEqual("const { ${1:a} } = require('${2:package}');");
  });

  it('require with 2 object properties', () => {
    expect(
      generate(abbreviationIDs.RQR, "const { a, b } = require('package');"),
    ).toEqual("const { ${1:a}, ${2:b} } = require('${3:package}');");
  });

  it('require with 3 object properties', () => {
    expect(
      generate(abbreviationIDs.RQR, "const { a, b, c } = require('package');"),
